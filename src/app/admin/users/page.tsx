        ...
        <div className="lg:col-span-3 rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium text-gray-600">Bots</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {users.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-600">{u.messages_used}</td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="rounded px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 disabled:opacity-50">Previous</button>
              <span className="text-sm text-gray-500">{page} / {totalPages}</span>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="rounded px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 disabled:opacity-50">Next</button>
            </div>
          )}
        </div>
          ...
          {detailLoading ? (
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">{selectedUser.profile.email}</p>
              ...
              <div className="rounded-lg border border-gray-200 p-3">
                <div className="text-xs font-medium text-gray-500 uppercase">Subscription</div>
              ...
              <div className="rounded-lg border border-gray-200 p-3">
                ...
                {selectedUser.bots.length === 0 ? (
                  <p className="mt-1 text-xs text-gray-400">No bots</p>
                ) : (
                  <div className="mt-1 space-y-1">
                    {selectedUser.bots.map((bot) => (
                      <div key={bot.id} className="flex items-center justify-between text-xs">
                        ...
                        <span className={bot.is_active ? 'text-green-600' : 'text-gray-400'}>
              ...
              {selectedUser.usage_records.length > 0 && (
                <div className="rounded-lg border border-gray-200 p-3">
                  <div className="text-xs font-medium text-gray-500 uppercase">Current Usage</div>
              ...
              {selectedUser.invoices.length > 0 && (
                <div className="rounded-lg border border-gray-200 p-3">
                  <div className="text-xs font-medium text-gray-500 uppercase">Recent Invoices</div>
                  <div className="mt-1 space-y-1">
...
function PlanBadge({ plan, status }: { plan: string; status: string }) {
  ...
  return (
    <span className="flex items-center gap-1">
      ...
      {status === 'canceled' && (
        <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs text-red-600">canceled</span>
      )}
    </span>
  );
}
