  ...
  return (
    <div className="max-w-3xl">
      ...
      <Section title={c.quickstart.title} id="quickstart">
        <p className="mt-2 text-sm text-gray-700">{c.quickstart.description}</p>
              ...
              {step.description ? (
                <p dangerouslySetInnerHTML={{
                  __html: step.description
                    .replace('<apikeys>', '<a href="/dashboard/api-keys" class="text-blue-600 hover:underline">')
                    .replace('</apikeys>', '</a>')
                }} />
              ) : (
  ...
  -d '{
    ...
    "message": "Hello, what is your return policy?"
      ...
      <Section title={c.authentication.title} id="authentication">
        <p className="mt-2 text-sm text-gray-700" dangerouslySetInnerHTML={{ __html: c.authentication.description }} />
        <CodeBlock>{`Authorization: Bearer ask_your_api_key_here`}</CodeBlock>

        <h3 className="mt-6 text-base font-semibold text-gray-800">{c.authentication.managementTitle}</h3>
      ...
      <Section title={c.codeExamples.title} id="code-examples">
        <h3 className="mt-4 text-base font-semibold text-gray-800">{c.codeExamples.pythonTitle}</h3>
...
`}</CodeBlock>
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.codeExamples.jsTitle}</h3>
...
`}</CodeBlock>
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.codeExamples.curlTitle}</h3>
        <CodeBlock lang="bash">{`curl -X POST https://your-domain.com/api/v1/chat \\
  -H "Authorization: Bearer ask_your_api_key_here" \\
  -H "Content-Type: application/json" \\
  -d '{
    ...
    "message": "What is your return policy?"
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.codeExamples.streamingTitle}</h3>
        <CodeBlock lang="python">{`import requests
...
`}</CodeBlock>
      ...
      <Section title={c.endpoints.title} id="endpoints">
        ...
        <Endpoint method="POST" path="/api/v1/chat">
          ...
          <h4 className="mt-3 text-sm font-semibold text-gray-700">{c.endpoints.chat.requestBodyTitle}</h4>
          <CodeBlock lang="json">{`{
  "bot_id": "uuid",
  "message": "What is your return policy?",
  ...
  "stream": false
          ...
          <h4 className="mt-3 text-sm font-semibold text-gray-700">{c.endpoints.chat.responseTitle}</h4>
          <CodeBlock lang="json">{`{
  "success": true,
  "data": {
    ...
    "usage": {
      ...
      "completion_tokens": 100
          ...
          <h4 className="mt-3 text-sm font-semibold text-gray-700">{c.endpoints.chat.streamingTitle}</h4>
          <p className="text-sm text-gray-600">{c.endpoints.chat.streamingDescription}</p>
  ...
  "data": {
    ...
    "usage": {
      ...
      <Section title={c.errors.title} id="errors">
        <h3 className="mt-4 text-base font-semibold text-gray-800">{c.errors.errorsTitle}</h3>
        <table className="mt-3 w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {c.errors.rows.map((row) => (
              <tr key={row.status}>
                <td className="py-2"><code>{row.status}</code></td>
        ...
        <p className="mt-3 text-sm text-gray-500" dangerouslySetInnerHTML={{ __html: c.errors.errorFormat }} />

        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.errors.rateLimitTitle}</h3>
        <table className="mt-3 w-full text-left text-sm">
          <thead>
            <tr className="border-b border-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {c.errors.rateLimitRows.map((row) => (
              <tr key={row.plan}>
                <td className="py-2 text-gray-700">{row.plan}</td>
                <td className="py-2 text-gray-700">{row.limit}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="mt-2 text-sm text-gray-500" dangerouslySetInnerHTML={{ __html: c.errors.rateLimitNote }} />
      </Section>

      {/* 6. SDK / Webhook Tips */}
      <Section title={c.tips.title} id="tips">
        <h3 className="mt-4 text-base font-semibold text-gray-800">{c.tips.envTitle}</h3>
...
ASKDOCS_BASE_URL=https://your-domain.com/api/v1`}</CodeBlock>
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.tips.errorHandlingTitle}</h3>
        <CodeBlock lang="python">{`import requests
...
`}</CodeBlock>
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.tips.multiTurnTitle}</h3>
        <p className="mt-2 text-sm text-gray-700" dangerouslySetInnerHTML={{ __html: c.tips.multiTurnDescription }} />
...
`}</CodeBlock>
        ...
        <h3 className="mt-8 text-base font-semibold text-gray-800">{c.tips.webhookTitle}</h3>
        <p className="mt-2 text-sm text-gray-700">{c.tips.webhookDescription}</p>
        <ul className="mt-2 list-inside list-disc space-y-1 text-sm text-gray-700">
